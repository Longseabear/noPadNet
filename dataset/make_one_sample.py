...
def process_func(idx):
    ...
    eye_avg = (lm[0] + lm[1]) * 0.5 + 0.5 # eye center pixel
    mouth_avg = (lm[3] + lm[4]) * 0.5 + 0.5 # mouse center pixel
    eye_to_eye = lm[1] - lm[0] # eye difference x,y
    eye_to_mouth = mouth_avg - eye_avg # mouth_av
    x = eye_to_eye - rot90(eye_to_mouth)
    x /= np.hypot(*x)
    x *= max(np.hypot(*eye_to_eye) * 2.0, np.hypot(*eye_to_mouth) * 1.8)
    y = rot90(x)
    c = eye_avg + eye_to_mouth * 0.1 # 눈 평균 위치에서, eye_to_mouth위치만큼 0.1 이동함
